...
COMMON_MEMBER_NAMES = frozenset(
    [
        ...
        "fCheckDebugFlags",
        "defaultPersistence",
        "describeSyntaxExecutable",
        "lReadStream",
        "prompt",
        "getNextExecuteCommand",
        "registerOperational",
        "registerActive",
        "registerEnum",
        "defineOption_1",
        ...
        "mapChargerB",
        "mapChargerC",
        "mapSecondaryPowerSupplyA",
        "mapSecondaryPowerSupplyB",
        "mapUInterfaceCabin",
        "mapUInterfaceRadio",
        "TransA",
        "TransB",
...
def build_clean_dependency_map(output, filter_stats, unified_data):
    ...
    lines.append(
        "| **high** | 0.95–1.00 | %d | Confirmed by both interface and implementation |"
        % conf["high"]["count"]
    )
    lines.append(
        "| **medium** | 0.70–0.94 | %d | Compiler-enforced interface declaration |"
        % conf["medium"]["count"]
    )
    lines.append(
        "| **low** | 0.30–0.69 | %d | Regex-inferred from implementation code |"
        % conf["low"]["count"]
    )
    lines.append("")
    ...
    lines.append(
        "| interface | %d | Declared in POU/DUT interface variables only |"
        % prov.get("interface", 0)
    )
    lines.append(
        "| implementation | %d | Found in ST implementation bodies only |"
        % prov.get("implementation", 0)
    )
    lines.append(
    ...
    lines.append(
        "| Total edges | %d | %d | -%.1f%% |"
        % (total_input, total_clean, reduction_pct)
    )
    lines.append(
        "| Total nodes | %d | %d | — |"
    ...
    lines.append("")
    lines.append(
        "The cleaned graph removes **%.1f%%** of edges that were identified"
        % reduction_pct
    )
    lines.append(
    ...
    if high_edges:
        ...
        for edge in sorted(
            high_edges, key=lambda e: (-e["confidence"], e["source"], e["target"])
        ):
            iface_ctx = (
                ", ".join("%s.%s" % (c[0], c[1]) for c in edge["interface_categories"])
                if edge["interface_categories"]
                else "—"
            )
            impl_ctx = (
                ", ".join(edge["implementation_categories"])
                if edge["implementation_categories"]
                else "—"
            )
            lines.append(
                "| `%s` | `%s` | %.2f | %s | %s |"
    ...
    if medium_edges:
        ...
        for edge in sorted(
            medium_edges, key=lambda e: (-e["confidence"], e["source"], e["target"])
        )[:50]:
            iface_ctx = (
                ", ".join("%s.%s" % (c[0], c[1]) for c in edge["interface_categories"])
                if edge["interface_categories"]
    ...
    for i, (node, fi, fo, hfi) in enumerate(impact_sorted[:30], 1):
        if fi > 0:
            lines.append("| %d | `%s` | %d | %d | %d |" % (i, node, fi, fo, hfi))
    lines.append("")
    ...
    print(
        ...
        % (
            ...
            (
                (
                    filter_stats["total_filtered"]
                    / filter_stats["total_input_edges"]
                    * 100
                )
                if filter_stats["total_input_edges"] > 0
                else 0
            ),
        )
    )
