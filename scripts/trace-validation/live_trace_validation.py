...
def main():
    ...
    top_level = [
        'CANBusDrive', 'System', 'Lift', 'BMS', 'Steering', 'Travel',
        'TMS', 'Charger', 'SecondaryPowerSupply', 'ProgrammedMovementControl',
        'SystemState', 'Spreader',
    ]
    ...
    for particle in top_level:
        ...
        if resp.startswith('__CONNECTION_ERROR__'):
            ...
            if parsed:
                ...
                print('    OK: %d children (%d op, %d nop)' % (total, len(op), len(nop)))
                child_names = []
                for c in op[:10]:
    ...
    representative_signals = {
        'CANOpenDrive-TransA': [
            'CANBusDrive/cTransA/Current', 'CANBusDrive/cTransA/MotorTemp',
            'CANBusDrive/cTransA/BattVoltage', 'CANBusDrive/cTransA/BattCurrent',
            'CANBusDrive/cTransA/CntrlTemp', 'CANBusDrive/cTransA/Velocity',
            'CANBusDrive/cTransA/Torque', 'CANBusDrive/cTransA/ProposedThrottle',
            'CANBusDrive/cTransA/TargetSpeed', 'CANBusDrive/cTransA/BatteryPower',
            'CANBusDrive/cTransA/NodeState', 'CANBusDrive/cTransA/HeartbeatCounter',
        ],
        'CANOpenDrive-SteerA': [
            'CANBusDrive/cSteerA/Current', 'CANBusDrive/cSteerA/MotorTemp',
            'CANBusDrive/cSteerA/Velocity', 'CANBusDrive/cSteerA/Torque',
            'CANBusDrive/cSteerA/TargetSpeed', 'CANBusDrive/cSteerA/Diagnostic',
            'CANBusDrive/cSteerA/Stalled', 'CANBusDrive/cSteerAngleA/Angle',
        ],
        'CANOpenDrive-WinchA': [
            'CANBusDrive/cWinchA/Current', 'CANBusDrive/cWinchA/MotorTemp',
            'CANBusDrive/cWinchA/Velocity', 'CANBusDrive/cWinchA/Torque',
            'CANBusDrive/cWinchA/ProposedThrottle', 'CANBusDrive/cWinchA/TargetSpeed',
        ],
        'CANOpenDrive-Spreader': [
            'CANBusDrive/cSpreader/TelescopingActive', 'CANBusDrive/cSpreader/TwistlocksEnabled',
            'CANBusDrive/cSpreader/HydraulicPressure', 'CANBusDrive/cSpreader/HydraulicTemperature',
            'CANBusDrive/cSpreader/BrakeDisengaged', 'CANBusDrive/cSpreader/PumpMotorTemperature',
        ],
        'System-State': [
            'System/SystemState', 'System/ChargingEnabled',
            'System/AllowDownTrace', 'System/AllowUpTrace',
        ],
        'BMS-Signals': [
            'BMS/SOC', 'BMS/Current', 'BMS/VoltageA', 'BMS/VoltageB',
            'BMS/Availability', 'BMS/CurrentState', 'BMS/RequestedState',
            'BMS/RestartCount',
        ],
        'Lift-Signals': [
            'Lift/Input', 'Lift/ScalingA', 'Lift/Diagnostic', 'Lift/ThrottleValue',
        ],
        'Steering-Signals': [
            'Steering/Input', 'Steering/TargetAngle', 'Steering/AdjustState',
            'Steering/SteerModeDiagnostic',
        ],
        'Travel-Signals': [
            'Travel/Input', 'Travel/Diagnostic', 'Travel/ThrottleValue', 'Travel/ScalingA',
        ],
        'TMS-Signals': [
            'TMS/RequestedState', 'TMS/CurrentState', 'TMS/CoolantReservoir',
            'TMS/CurrentCoolantTemperature',
        ],
        'Charger-Signals': [
            'Charger/ChargerDiagnostic', 'Charger/ChargingCurrentTarget',
            'Charger/ChargingVoltageTarget',
        ],
        'SecondaryPowerSupply': [
            'SecondaryPowerSupply/CurrentState', 'SecondaryPowerSupply/RequestedState',
            'SecondaryPowerSupply/Availability',
        ],
        'ProgrammedMovement': [
            'ProgrammedMovementControl/Movement/Code', 'ProgrammedMovementControl/Movement/Active',
            'ProgrammedMovementControl/Movement/State', 'ProgrammedMovementControl/Requested',
        ],
        'Risky-Abbreviations': [
            'CANBusDrive/cTransA/BattVoltage', 'CANBusDrive/cTransA/BattCurrent',
            'CANBusDrive/cTransA/CntrlTemp', 'CANBusDrive/cWinchA/BattVoltage',
            'CANBusDrive/cWinchA/BattCurrent', 'CANBusDrive/cSteerA/BattVoltage',
            'CANBusDrive/cSteerA/BattCurrent',
        ],
        'Risky-Typo': [
            'Travel/MovementD/LDiagnostic', 'Travel/MovementD/Diagnostic',
        ],
        'Risky-Uncertain': [
            'CANBusDrive/cWinchA/ProposedThrottle', 'CANBusDrive/cWinchA/TargetSpeed',
            'CANBusDrive/cTransA/ProposedThrottle', 'CANBusDrive/cTransA/TargetSpeed',
        ],
    }
    ...
    for category, paths in representative_signals.items():
        ...
        if not sock:
            ...
            for p in paths:
                RESULTS['not_tested'].append({'path': p, 'category': category, 'reason': 'connection_failed'})
            continue
    ...
    canbus_success = [r for r in RESULTS['describe_success'] if r['category'].startswith('CANOpenDrive')]
    if canbus_success:
        transa_signals = [r['path'].split('/cTransA/')[1] for r in canbus_success if '/cTransA/' in r['path']]
        if transa_signals:
            print('\n  Successful transA signals (%d): %s' % (len(transa_signals), transa_signals))
            drive_prefixes = ['cTransB', 'cTransC', 'cTransD',
                            'cSteerB', 'cSteerC', 'cSteerD',
                            'cWinchB', 'cWinchC', 'cWinchD',
                            'cWinchAngleB', 'cWinchAngleC', 'cWinchAngleD',
                            'cSteerAngleB', 'cSteerAngleC', 'cSteerAngleD']
            for prefix in drive_prefixes:
                for sig in transa_signals:
                    ...
                    RESULTS['inferred_from_pattern'].append({
                        ...
                        'confidence': 'high' if prefix.startswith('cTrans') or prefix.startswith('cSteer') or prefix.startswith('cWinch') else 'medium'
                    })
            print('  Inferred %d signals from transA pattern' % (len(drive_prefixes) * len(transa_signals)))
    ...
    for path, interval in metric_test_signals:
        ...
        if success:
            RESULTS['metric_register_success'].append({'path': path, 'interval': interval})
            print('    SUCCESS (response: %s)' % detail)
        else:
            RESULTS['metric_register_failure'].append({'path': path, 'interval': interval, 'detail': str(detail)[:100]})
    ...
    if emit_ok:
        RESULTS['emit_success'].append({'port': EMIT_PORT, 'data_preview': emit_detail[:300]})
    ...
    for r in RESULTS['describe_success']:
        ...
        if 'BattVoltage' in path:
            RESULTS['naming_corrections'].append({'path': path, 'correction': 'BattVoltage (not BatteryVoltage)'})
        if 'BattCurrent' in path:
            RESULTS['naming_corrections'].append({'path': path, 'correction': 'BattCurrent (not BatteryCurrent)'})
        if 'CntrlTemp' in path:
            RESULTS['naming_corrections'].append({'path': path, 'correction': 'CntrlTemp (not ControllerTemperature)'})

    corrections = list({c['correction']: c for c in RESULTS['naming_corrections']}.values())
    for c in corrections:
        print('  %s: %s' % (c['path'], c['correction']))

    # Save results
    output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'live_trace_validation_results.json')
    output = {
        'timestamp': time.strftime('%Y-%m-%d %H:%M:%S'),
        'target': PLC_HOST,
        'plc_reachable_ro': True,
        'plc_reachable_rw': len(RESULTS['metric_register_success']) > 0 or len(RESULTS['metric_register_failure']) > 0,
