...
def map_single_signal(raw_path):
    ...
    var_parts = cleaned[2:]
    if var_parts and var_parts[-1] in [
        "Value",
        "lValue",
        "_Unique",
        "Unique",
...
def main():
    ...
    if os.path.exists(MAP_FILE):
        ...
        with open(STATE_FILE, "r", encoding="utf-8") as f:
            ...
            active_keys = set(flat_map.keys())
        print(
            f"[+] Đã tải {len(active_keys)} node từ explorer_state.json (phục hồi tạm thời)."
        )
    else:
    ...
    print(
        f"[+] Tổng số node tìm kiếm mở rộng (gồm cả các lá con): {len(extended_keys)}"
    )
            ...
            for line in lines[1:]:
                ...
                if len(parts) >= 1 and parts[0]:
                    ...
                    trace_signals[fname].append(
                        {"raw_path": raw_path, "sample_time": sample_time}
                    )
                    total_raw_signals += 1
    ...
    for raw_path in sorted(unique_raw_paths):
        ...
        if mapping_progress % 50 == 0:
            print(
                f"[*] Đang đối chiếu tín hiệu {mapping_progress}/{len(unique_raw_paths)}..."
            )
    ...
    with open(OUTPUT_MD, "w", encoding="utf-8") as f:
        f.write(
            f"""# Báo Cáo Đối Chiếu Tín Hiệu Trace Config (Trace Config Mapping Report)
...
"""
        ...
        for raw, val in sorted(reconciliation_db.items()):
            ...
            status_badge = (
                "🟢 ACTIVE" if "VERIFIED_ACTIVE" in val["status"] else "🔴 DEAD"
            )
            f.write(
                f"| `{raw}` | {m_path} | `{val['type']}` | `{val['subsystem']}` | `{method}` | {status_badge} |\n"
            )
