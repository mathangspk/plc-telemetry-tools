...
def generate_markdown_hierarchy(tree, depth=0):
    ...
    if tree.get("status") == "[LEAF]":
        ...
        subsys_info = f" [Subsystem: {tree['subsystem']}]" if tree.get("subsystem") else ""
        lines.append(f"{indent}- 📂 **{short_name}** (`{tree.get('type', '')}`){subsys_info}")
...
def generate_mermaid_highlevel(tree):
    lines = ["graph TD", "    %% Style Definitions", "    classDef subsystem fill:#1e293b,stroke:#3b82f6,stroke-width:2px,color:#f8fafc;", "    classDef leaf fill:#0f172a,stroke:#10b981,stroke-width:1px,color:#34d399;"]
            ...
            if "children" in c:
                for gc in c["children"]:
                    ...
                    if gc.get("status") != "[LEAF]" and len(gc.get("children", [])) > 0:
                        lines.append(f"    {c_short} --> {c_short}_{gc_short}[📂 {gc_short}]")
...
def generate_mermaid_subsystem(tree, subsystem_name):
    ...
    lines = [f"graph TD", f"    %% Subsystem: {subsystem_name}", "    classDef folder fill:#1e293b,stroke:#3b82f6,stroke-width:2px,color:#f8fafc;", "    classDef variable fill:#0f172a,stroke:#10b981,stroke-width:1px,color:#e2e8f0;"]
    ...
    def traverse_sub(node, parent_id):
        ...
        if node.get("status") == "[LEAF]":
            lines.append(f"    {parent_id} --> {curr_id}[\"🍃 {node_short} ({node.get('type')})\"]")
...
def main():
    ...
    print(f"[+] Thống kê: Tổng số node = {total}, Lá (Leaf) = {leaves}, Nhánh (Expandable) = {expandable}")
