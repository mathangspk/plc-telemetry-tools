...
def translate_name(raw_name):
    """Translate CODESYS internal name to TCP Identity path with runtime corrections."""
...
for dev in [
    "BMSA",
    "BMSB",
    "SecondaryPowerSupplyA",
    "SecondaryPowerSupplyB",
    "UInterfaceCabin",
    "ChargerA",
]:
...
for dev in [
    ...
    for sig in [
        ...
        "Stalled",
        "MeasuredThrottle",
        "ZeroCurrentVelocityIndicator",
        "ZeroTargetSpeedIndicator",
        "StalledCount",
        "StalledIndicator",
        "StallIndicator",
        "StallInput",
...
# Drive device name remaps (TransA -> cTransA, Spreader -> cSpreader, etc.)
drive_device_remaps = [
...
for raw_sig in sorted(raw_signals):
    ...
    # Strip .Value and _Unique suffixes
    ...
    for old_path, new_path in drive_device_remaps:
        if runtime_path == old_path or runtime_path.startswith(old_path + "/"):
            ...
            # Now check drive abbreviations on the new path
    ...
    # Check drive signal abbreviations (for paths already using c-prefixed devices)
