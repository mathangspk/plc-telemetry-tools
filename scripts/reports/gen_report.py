...
subsystem_data = {
    'System/BMSAB': {
        'nop_children': ['AvailFactor','Availability','BMSAvailInsuff','BMSAvailLmtd','BMSChrgCutb',
            'BMSChrgMismatch','BMSCurrMismatch','BMSDischCutb','BMSNotConnect','BMSNotConnect',
            'BMSSOCMismatch','BMSVoltMismatch','Current','CurrentMismatch','CycleOutput','CycleOutput',
            'DsblConnect','DsblVoltMisMgmt','EnblBMS','EnblRestart','EnblSOCRecovery','RestartCount',
            'SOC','SOCLowOpMin','SOCLowStdbyMin','SOCMismatch','SOCRecCurrMin','SOCRecTimRst',
            'SOCWarnLow','TMSMismatch','Voltage','VoltageMismatch']
    },
    'System/Steer': {
        'op_children': ['CntxtQualModEx', 'iMovement'],
        'iMovement_children': ['ChannelMask','Override','Suppress','SuppressAfter','SuppressCntdwn','iMovement']
    },
    'System/TMS': {
        'op_children': ['Chilling','ClntReservoir','ClntTemp','CurrState','Heating','Pump'],
        'nop_children': ['BMSTMSCooling','BMSTMSHeating','BMSTMSNoCirc','Chilling','ClntChilling',
    ...
    'System/Travel': {
        'op_children': ['CntxtQualModEx', 'iMovement'],
        'iMovement_children': ['ChannelMask','ClntTempColdCutb','CntxtQualModEx','Life','Override',
            'Suppress','SuppressAfter','SuppressCntdwn','iMovement']
    },
    'System/Lift': {
        'op_children': ['CntxtQualModEx', 'iMovement'],
        'iMovement_children': ['ChannelMask','ClntTempColdCutb','CntxtQualModEx','Life','Override',
    ...
    'System/Secondary': {
        'nop_children': ['CntxtSgnMod','SecPwrMismatch','SecPwrNotRun','SecPwrNotRunMod','SecPwrWait']
    },
    'System/ProgMovCntrl': {
        'nop_children': ['ProgMovNull','steer_0']
    },
    'System/ChargerABC': {
        'nop_children': ['ChrgConn','ChrgDiscon','ChrgOBCErr','ChrgStdby','EnblManualChrg','ManualChrgCurr','ManualChrgVolt']
    },
    'System/Spreader': {
        'op_children': ['AB','CD','TwstALow','TwstBLow','TwstCLow','TwstDLow']
    },
    'System/SystemState': {
        'nop_children': ['AutoStart','CntxtSysCntrl','CntxtSysStateDn','CntxtSysStateUp','Event','State']
    },
    'System/FuncModeCntrl': {
        'nop_children': []
    },
    'CANBusDrive': {
        'devices': ['BusPower','cSpreader','cSteerA','cSteerAngleA','cSteerAngleB','cSteerAngleC',
    ...
    'CANBusDrive/cTransA': {
        'signals': ['Alarm','BattCurrent','BattPower','BattVoltage','CntrlTemp','Current','MotorTemp','TPO1Stuffing','Velocity']
    },
    'CANBusDrive/cSteerA': {
        'signals': ['Alarm','BattCurrent','BattPower','BattVoltage','CntrlTemp','Current','MotorTemp','TPO1Stuffing','Velocity']
    },
    ...
    'CANBusDrive/cSpreader': {
        'signals': ['Brake','BrakeDisengage','BrakeThrottle','HydPressure','HydTemp','HydraulicFilter',
...
known_nonexistent.update([
    'CANBusDrive/cSteerA/Diagnostic','CANBusDrive/cSteerB/Diagnostic',
    'CANBusDrive/cSteerC/Diagnostic','CANBusDrive/cSteerD/Diagnostic',
...
with open(md_path, 'w', encoding='utf-8') as f:
    ...
    f.write("**Scope:** %d unique signals from %d trace files\n" % (len(raw_signals), num_files))
        ...
        if sub == 'CANBusDrive':
            ...
            for key in ['op_children', 'nop_children', 'iMovement_children']:
                if key in info and info[key]:
                    parts_list.append('%s: %s' % (key, ', '.join(sorted(set(info[key])))))
            if parts_list:
                f.write("- %s\n\n" % '\n- '.join(parts_list))
            else:
                f.write("- (empty)\n\n")
    ...
    f.write("| # | Trace Config | Translated | Runtime Path | Rule |\n|---|---|---|---|---|\n")
    ...
    f.write("1. **ChargerABC signal mapping** - Particle exists with 7 children (ChrgConn, ChrgDiscon, ChrgOBCErr, ChrgStdby, EnblManualChrg, ManualChrgCurr, ManualChrgVolt). Trace config references ChargerDiagnostic, ChargingCurrentTarget, ChargingVoltageTarget which do NOT match any runtime child.\n")
    f.write("2. **Secondary signal mapping** - Particle exists with 5 children (CntxtSgnMod, SecPwrMismatch, SecPwrNotRun, SecPwrNotRunMod, SecPwrWait). Trace config references CurrentState, RequestedState, Availability, Controller* paths which do NOT match.\n")
    f.write("3. **ProgMovCntrl signal mapping** - Only 2 children (ProgMovNull, steer_0). Trace config expects Movement/Code, Movement/Active, Requested, etc. which do NOT exist.\n")
    f.write("4. **CANBusSystem children** - Not fully explored; trace config references BMSA, BMSB, SecondaryPowerSupplyA/B, UInterfaceCabin, ChargerA node states under CANOpenMasterSystem.\n")
    f.write("5. **Interlock particles** - Many Lift*Interlock signals in trace config; runtime particle names not confirmed (may be under System as separate particles).\n")
    f.write("6. **CANBusDrive signals beyond 9 confirmed** - Only 9 signals confirmed per drive (Alarm, BattCurrent, BattPower, BattVoltage, CntrlTemp, Current, MotorTemp, TPO1Stuffing, Velocity). Prior phases reported 34-child structure; current PLC state may differ.\n")
    f.write("7. **Steering wheel-level signals** - Trace config references Steering.WheelA/B/C/D sub-structures extensively; runtime System/Steer only shows CntxtQualModEx and iMovement.\n")
    f.write("8. **Lift children beyond iMovement** - Trace config references ScalingA-D, MovementA-D, Interlock, ChargeInterlock; runtime only shows CntxtQualModEx and iMovement.\n")
    f.write("9. **Travel children beyond iMovement** - Trace config references ScalingA-D, MovementA-D, Diagnostic; runtime only shows CntxtQualModEx and iMovement.\n")
    f.write("10. **Metric registration & emission** - Not verified in this pass.\n")
    f.write("11. **BMSAB Output** - Has 32 nop-children but trace config references paths like BMS.RestartCount, BMS.SOCRecoveryTimer which need mapping to BMSAB children.\n")
    f.write("12. **Spreader AB/CD children** - System/Spreader has AB, CD, TwstALow, etc. but trace config references CANBusDrive/cSpreader paths.\n")
    f.write("\n---\n\n")
...
print('[+] Exact: %d, Remapped: %d, NoMatch: %d, Nonexistent: %d' % (len(exact_match), len(remapped_abbrev), len(no_match), len(nonexistent_confirmed)))
