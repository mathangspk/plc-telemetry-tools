...
with open(md_path, 'w', encoding='utf-8') as f:
    ...
    f.write("**Scope:** %d unique signals from 28 trace files\n" % rec['total_unique_signals'])
    ...
    f.write("**Note:** PLC connection is unstable; sessions drop after 1-3 commands. Each subsystem probed in separate connection.\n\n")

    f.write("## 2. Top-Level System Children (Live-Enumerated)\n\n")
    f.write("Total: 60 children (59 op, 1 nop)\n\n")
    f.write("| Name | Type |\n|---|---|\n")
    sys_children = [
        ('Local', 'Local'), ('Reporting', 'Reporting'), ('LED', 'LED'), ('Executor', 'Executor'),
        ('Cycle', 'Cycle'), ('PLC', 'PLC'), ('SystemState', 'SystemState'),
        ('ParserRW', 'ParseTCPServer'), ('ParserRO', 'ParseTCPServer'), ('ParserEdge', 'ParseTCPServer'),
        ('Metric0ms', 'ParticleRepEvent'), ('Metric250ms', 'ParticleRepEvent'),
        ('Metric500ms', 'ParticleRepEvent'), ('Metric1s', 'ParticleRepEvent'),
        ('Metric5s', 'ParticleRepEvent'), ('Metric1m', 'ParticleRepEvent'),
        ('Metric1h', 'ParticleRepEvent'), ('Metric1d', 'ParticleRepEvent'),
        ('CANBusSystem', 'CANMaster'), ('CANBusDrive', 'CANMaster'),
        ('Cabin', 'UICabin'), ('CabinSecPwr', 'oSysStateAuto'),
        ('BMSAB', 'BMSAB'), ('ChargerABC', 'ChargerABC'),
        ('FuncModeCntrl', 'FuncModeCntrl'), ('UIEvent', 'UIEvent'),
        ('ProgMovCntrl', 'mProgCntrl'), ('Travel', 'mTravelQuad'),
        ('Lift', 'mLiftQuad'),
        ('iLiftAPos', 'iwScaled'), ('iLiftBPos', 'iwScaled'),
        ('iLiftCPos', 'iwScaled'), ('iLiftDPos', 'iwScaled'),
        ('iLiftABSync', 'iSynch'), ('iLiftCDSync', 'iSynch'), ('iLiftSync', 'iSynch'),
        ('Spreader', 'Spreader'),
        ('iTwistALowered', 'iwPosOnly'), ('iTwistBLowered', 'iwPosOnly'),
        ('iTwistCLowered', 'iwPosOnly'), ('iTwistDLowered', 'iwPosOnly'),
        ('iwTwistRdy', 'iwTwistRdy'), ('iTwistLowerTrav', 'iwLockout'),
        ('OperatingMode', 'OperatingMode'), ('Active', 'ActiveSystem'),
        ('UMFS', 'UMFS'), ('EstopLoad', 'oSysStateAuto'),
        ('TMS', 'TMS'), ('Secondary', 'Secondary'),
        ('Alarm', 'oSystemState'), ('Beacons', 'oSystemState'),
        ('Horn', 'oSystemState'), ('Lights', 'oSystemState'),
        ('SensorSupply', 'oSysStateAuto'),
        ('Steer', 'mSteerElec'),
        ('iSteerAPos', 'iwScaled'), ('iSteerBPos', 'iwScaled'),
        ('iSteerCPos', 'iwScaled'), ('iSteerDPos', 'iwScaled'),
    ...
    f.write("| System/ProgrammedMovementControl | System/ProgMovCntrl | Live-enumerated |\n\n")
    ...
    f.write("Trace config references: Input, InputProcessed, ScalingA-D, Diagnostic, MovementA-D/Diagnostic, Interlock*, ChargeInterlock*, ThrottleValue\n")
    f.write("Runtime shows: CntxtQualModEx, iMovement (with ChannelMask, Override, Suppress, etc.)\n")
    f.write("**Gap:** Lift has many more children than currently enumerated; Input, ScalingA-D, Interlock, ChargeInterlock particles not found.\n\n")

    f.write("### 7.2 Lift*Interlock Particles (24 signals)\n")
    f.write("Trace config references: LiftAPositionInterlock, LiftBPositionInterlock, LiftCPositionInterlock, LiftDPositionInterlock, LiftABSynchronousInterlock, LiftCDSynchronousInterlock, LiftSynchronousInterlock\n")
    f.write("Runtime: These particles not found as top-level System children.\n\n")

    f.write("### 7.3 Travel Subsystem (10 signals)\n")
    f.write("Trace config references: Diagnostic, MovementA-D/Diagnostic, ScalingA-D, Input, InputProcessed, etc.\n")
    f.write("Runtime shows: CntxtQualModEx, iMovement\n\n")

    f.write("### 7.4 Steering Subsystem (40+ signals)\n")
    f.write("Trace config references: Input, TargetAngle, AdjustState, WheelA-D/*, SteerModeDiagnostic, ModeChange, etc.\n")
    f.write("Runtime shows: CntxtQualModEx, iMovement\n\n")

    f.write("### 7.5 BMSAB Deep Signals (10 signals)\n")
    f.write("Trace config references: RestartCount, SOCRecoveryTimer, SwitchingStateTimer, TargetStateA/B, ControllerBMSA/B, etc.\n")
    f.write("Runtime BMSAB has 32 nop-children but these specific paths not matched.\n\n")

    f.write("### 7.6 ChargerABC Mismatch (7 signals)\n")
    f.write("Trace config references: ChargerDiagnostic, ChargingCurrentTarget, ChargingVoltageTarget, CurrentBMSChargeState, RequestedBMSChargeState, RequestedOBCState, CurrentOBCState\n")
    f.write("Runtime ChargerABC has: ChrgConn, ChrgDiscon, ChrgOBCErr, ChrgStdby, EnblManualChrg, ManualChrgCurr, ManualChrgVolt\n\n")

    f.write("### 7.7 Secondary Mismatch (12 signals)\n")
    f.write("Trace config references: CurrentState, RequestedState, Availability, ControllerSecondaryPowerSupplyA/B/*\n")
    f.write("Runtime Secondary has: CntxtSgnMod, SecPwrMismatch, SecPwrNotRun, SecPwrNotRunMod, SecPwrWait\n\n")

    f.write("### 7.8 ProgMovCntrl Mismatch (10 signals)\n")
    f.write("Trace config references: Movement/Code, Movement/ProgMovDiagnostic, Requested, Movement/Enabled, Movement/Diagnostic, Movement/Active, Movement/TimeStep, Movement/State, Movement/Input, Movement/Moving, Movement/Success, Movement/ThrottleValue\n")
    f.write("Runtime ProgMovCntrl has only: ProgMovNull, steer_0\n\n")

    f.write("### 7.9 SystemState Deep Signals (4 signals)\n")
    f.write("Trace config references: AllowDownTrace, AllowUpTrace, NextStateTrace, SystemControlTrace, ForceMaxTrace\n")
    f.write("Runtime SystemState has: AutoStart, CntxtSysCntrl, CntxtSysStateDn, CntxtSysStateUp, Event, State\n\n")

    f.write("### 7.10 CANBusDrive/cSpreader Missing Signals (3 signals)\n")
    f.write("Trace config references: TelescopingEnabled (duplicate), TwistlocksEnabled (duplicate)\n")
    f.write("Runtime cSpreader has TelscpEnbl, TwstEnbl but some trace config variants not matched.\n\n")
    ...
    f.write("1. **Lift deep hierarchy** - Input, ScalingA-D, MovementA-D, Interlock, ChargeInterlock particles not found in runtime. May require deeper traversal or different path.\n")
    f.write("2. **Lift*Interlock particles** - 7 interlock particles referenced in trace config not found as System children. May be nested or named differently.\n")
    f.write("3. **Travel deep hierarchy** - Diagnostic, ScalingA-D, MovementA-D/Diagnostic not found. Only CntxtQualModEx and iMovement confirmed.\n")
    f.write("4. **Steering deep hierarchy** - 40+ signals referencing Steering.Input, Steering.TargetAngle, Steering.WheelA-D/* not matched. Only Steer.CntxtQualModEx and Steer.iMovement confirmed.\n")
    f.write("5. **BMSAB deep signals** - RestartCount, SOCRecoveryTimer, SwitchingStateTimer, TargetStateA/B, ControllerBMSA/B paths not matched to BMSAB children.\n")
    f.write("6. **ChargerABC signal mapping** - Trace config names (ChargerDiagnostic, ChargingCurrentTarget, etc.) do not match runtime children (ChrgConn, ChrgOBCErr, etc.).\n")
    f.write("7. **Secondary signal mapping** - Trace config names (CurrentState, RequestedState, Availability, Controller*) do not match runtime children (SecPwrNotRun, SecPwrWait, etc.).\n")
    f.write("8. **ProgMovCntrl signal mapping** - Trace config expects Movement/* hierarchy; runtime only has ProgMovNull and steer_0.\n")
    f.write("9. **SystemState deep signals** - AllowDownTrace, AllowUpTrace, NextStateTrace, SystemControlTrace, ForceMaxTrace not found.\n")
    f.write("10. **CANBusDrive signals beyond 9** - Only 9 signals confirmed per drive; prior phases reported 34-child structure. Current PLC state may differ or connection truncation.\n")
    f.write("11. **Metric registration & emission** - Not verified in this pass.\n")
    f.write("12. **CANBusSystem full hierarchy** - Only BMSA, BMSB, ChargerA children partially confirmed.\n")

    f.write("\n---\n\n")
    f.write("*End of Phase 4 Runtime-First Validation & Remapping Report.*\n")

print('[+] Markdown report saved: %s' % md_path)
print('[+] Summary: %d exact, %d remapped, %d no-match, %d nonexistent' % (len(exact), len(remapped), len(no_match), len(nonexistent)))
