...
known_nonexistent.update(
    [
        ...
        "System/Travel/MovementD/LDiagnostic",
        "System/Travel/MovementD/Diagnostic",
        "System/Travel/MovementD/lDiagnostic",
        "System/Lift/Diagnostic",
        "System/Travel/Diagnostic",
    ]
)
...
for raw_sig in sorted(raw_signals):
    ...
    if (
        len(parts) >= 3
        and parts[0] in ["CANBusDrive", "System"]
        and any(p.startswith("c") for p in parts[1:])
    ):
        leaf = parts[-1]
        for long_name, short_name in drive_abbrevs:
            if leaf == long_name:
