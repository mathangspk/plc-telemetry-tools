...
class TraceApp:
    ...
    def setup_ui(self):
        ...
        tk.Entry(frame1, textvariable=self.json_file_path, state='readonly').pack(side="left", fill="x", expand=True)
        tk.Button(frame1, text="Browse", command=self.browse_json).pack(side="right", padx=5)
        
        # --- Folder Selection ---
        tk.Label(self.root, text="2. Chọn thư mục lưu kết quả:").pack(anchor="w", **pad)
        frame2 = tk.Frame(self.root)
        frame2.pack(fill="x", **pad)
        tk.Entry(frame2, textvariable=self.output_dir_path, state='readonly').pack(side="left", fill="x", expand=True)
        tk.Button(frame2, text="Browse", command=self.browse_output).pack(side="right", padx=5)
        ...
        self.btn_start = tk.Button(frame3, text="Start Data Collection", bg="green", fg="white", font=("Arial", 12, "bold"), command=self.start_collection)
        self.btn_start.pack(side="left", expand=True, fill="x", padx=10)
        
        self.btn_stop = tk.Button(frame3, text="Stop Data Collection", bg="red", fg="white", font=("Arial", 12, "bold"), command=self.stop_collection, state=tk.DISABLED)
        self.btn_stop.pack(side="right", expand=True, fill="x", padx=10)
        
        # --- Open Folder Button ---
        frame4 = tk.Frame(self.root)
        frame4.pack(fill="x", pady=10)
        tk.Button(frame4, text="Mở thư mục kết quả (Folder)", font=("Arial", 10), command=self.open_output_folder).pack(expand=True, fill="x", padx=50)

    def browse_json(self):
        file_path = filedialog.askopenfilename(
            title="Chọn file JSON config",
    ...
    def _start_task(self):
        ...
        if os.path.exists(start_script):
            subprocess.run(["python", start_script, "--trace-dir", trace_dir, trace_name], shell=True)
        else:
        ...
        try:
            req = urllib.request.Request(f"http://{EDGE_DEVICE_IP}:{EDGE_DEVICE_PORT}/start", method="POST")
            with urllib.request.urlopen(req, timeout=5) as response:
                data = json.loads(response.read().decode())
                if data.get("status") == "started":
                    self.current_remote_file = data.get("file")
                    self.log(f"Đang ghi! (Remote file: {self.current_remote_file})", "green")
                else:
    ...
    def _stop_task(self):
        ...
        try:
            req = urllib.request.Request(f"http://{EDGE_DEVICE_IP}:{EDGE_DEVICE_PORT}/stop", method="POST")
        ...
        if os.path.exists(stop_script):
            subprocess.run(["python", stop_script, "--trace-dir", trace_dir, trace_name], shell=True)
