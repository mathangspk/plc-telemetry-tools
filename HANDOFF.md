...
- **CAN Interface for Apollo 4:** Đã tạo tệp tài liệu `Apollo 4.xlsx` và `bms_pdo_mapping.md` để đặc tả dữ liệu CAN interface riêng cho BMSA và BMSB (tách thành 2 sheet riêng biệt). Đã đính chính lại mapping: Nhiệt độ BMS là kiểu USINT (1 byte), và cập nhật Node ID cuối cùng (BMSA=40, BMSB=42) cùng Start/End Address dải COB-ID chuẩn xác.
