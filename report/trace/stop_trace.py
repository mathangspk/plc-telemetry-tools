#!/usr/bin/env python
"""stop_trace.py — Unregister all signals from a trace JSON config.

Connects to the PLC's RW port (default 49870) and unregisters each signal
from the trace config using the metric unregister command.

Usage:
    python scripts/telemetry/stop_trace.py CanBusDrive
    python scripts/telemetry/stop_trace.py CanBusDrive --dry-run
    python scripts/telemetry/stop_trace.py TraceSteer -H 10.2.3.99
"""
...
def execute_metric_clear(host, metric, port=49870):
    """Connects to RW port and executes clear command on the metric."""
    print("  Executing '{} clear' command on RW port {}...".format(metric, port))
    sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    sock.settimeout(3.0)
    try:
        sock.connect((host, port))
        wait_for_prompt(sock, timeout=3.0)
        cmd = "{} clear\r\n".format(metric)
        sock.sendall(cmd.encode("ascii"))
        wait_for_prompt(sock, timeout=3.0)
        graceful_close(sock)
        print("    [OK] Clear successful.")
    except Exception as e:
        print("    [ERROR] Clear command failed: {}".format(e))
        try:
            sock.close()
        except OSError:
            pass
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Unregister all signals from a trace JSON config."
    )
    parser.add_argument(
        "name",
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Print what would be unregistered without connecting",
    )
    parser.add_argument(
        "--timeout",
        type=int,
    ...
    if args.dry_run:
        ...
        for i, sig in enumerate(signals, 1):
            ...
            cmd = "{} unregister object={}".format(sig_metric, sig["path"])
    ...
    # Verify active emission on port 49890 (expecting silence)
    print()
