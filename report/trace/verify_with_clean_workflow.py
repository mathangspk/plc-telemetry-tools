...
def recv_until_terminator(sock, timeout):
    ...
    while time.monotonic() < deadline:
        ...
        if idx != -1:
            decoded = (
                buf[:idx].decode("utf-8", errors="replace").replace("\x00", "").strip()
            )
...
def check_active_emission(host):
    ...
    try:
        ...
        if data:
            ...
            print(
                f"    [WARN] Active emission detected! Received {len(data)} additional bytes."
            )
...
def check_registered_members(host, target_metrics):
    ...
    for metric in target_metrics:
        ...
        if sock:
            ...
            try:
                raw_resp, resp_text = send_command(
                    sock, f"{metric} describe", timeout=2.0
                )
                ...
                if confirmed_paths:
                    print(
                        f"    [WARN] Metric '{metric}' has {len(confirmed_paths)} active members."
                    )
    ...
    if active_metrics_with_members:
        print(
            f"    [WARN] Metrics with active members in describe: {list(active_metrics_with_members)}"
        )
    else:
...
def enforce_silence_workflow(host, target_metrics):
    ...
    if all_active_metrics:
        print(
            f">>> [ALERT] Active metrics detected: {list(all_active_metrics)}! Triggering reset clear..."
        )
        ...
        if still_active:
            print(
                f">>> [WARN] PLC remains active on metrics {list(still_active)} after clear command!"
            )
            print(
                ">>> [RETRY] Attempting one more clear command on still-active metrics..."
            )
            ...
            if final_active:
                print(
                    f">>> [FATAL] PLC state could not be set to silent on metrics {list(final_active)}! Check PLC health."
                )
                return False
...
def run_focused_test(host, trace_file, listen_duration=EMIT_LISTEN_DURATION):
    ...
    print(
        f"  Target Metrics: {', '.join(metrics_in_trace)} (Temporarily overridden from: {', '.join(sorted(list(original_metrics)))})"
    )
    ...
    for chunk_idx, chunk_signals in enumerate(chunks, 1):
        ...
        if not enforce_silence_workflow(host, metrics_in_trace):
            print(
                "  [WARN] Post-test cleanup did not result in absolute silence! Check PLC status."
            )
        else:
    ...
    print(
        f"  Status Summary: {len(active_signals)} Active | {len(silent_signals)} Silent | {len(failed_signals)} Failed"
    )
    print("=" * 80)
    ...
    with open(report_file_md, "w", encoding="utf-8") as f:
        ...
        f.write(
            f"- **Configured Metrics in Files:** {', '.join(sorted(list(original_metrics)))}\n"
        )
        f.write(
            f"- **Verification Metric (Overridden):** Metric250ms (for active telemetry validation)\n\n"
        )
        ...
        tot = len(signals)
        f.write(
            f"| **Active (PASS_ACTIVE)** | {len(active_signals)} | {len(active_signals)/tot*100:.1f}% |\n"
        )
        f.write(
            f"| **Silent (SILENT)** | {len(silent_signals)} | {len(silent_signals)/tot*100:.1f}% |\n"
        )
        f.write(
            f"| **Failed (FAIL)** | {len(failed_signals)} | {len(failed_signals)/tot*100:.1f}% |\n"
        )
        ...
        if active_signals:
            ...
            for s in sorted(active_signals, key=lambda x: x["name"]):
                f.write(
                    f"| `{s['name']}` | `{s['path']}` | {s['registered']} | {s['described']} | {s['emitted']} |\n"
                )
        else:
    ...
    print(
        f"Beautiful Markdown report saved to: [Report](file:///{report_file_md.replace(chr(92), '/')})"
    )
    return {
        "trace": trace_name,
...
def main():
    parser = argparse.ArgumentParser(
        description="Focused sequential trace verification with pre/post clean workflow."
    )
    parser.add_argument(
        "-t",
        "--trace",
        default="cTransTest",
        help="Name of the trace configuration file to test",
    )
    ...
    parser.add_argument(
        ...
        "--duration",
        type=float,
        default=EMIT_LISTEN_DURATION,
        help="Duration to listen to emit stream in seconds",
    )
    parser.add_argument(
        "--trace-dir", default=TRACE_DIR, help="Directory containing trace JSON files"
    )
